# The model from chessi-1.keras should play against the user.
# There is a GUI to see the game and allow user interaction.

import chess
import pygame
import numpy as np
import time
from catmate import run, prepare
...
def main():
    ...
    while running:
        ...
        if not board.is_game_over() and not player_turn:
            time.sleep(0.5)
            move = chess.Move.from_uci(run(board=board, depth=DEPTH))
            if move in board.legal_moves:
                board.push(move)
            player_turn = True
